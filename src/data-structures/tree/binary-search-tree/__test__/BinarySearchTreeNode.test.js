...
describe('BinarySearchTreeNode', () => {
  ...
  it('should insert nodes in correct order', () => {
    ...
    expect(bstNode.contains(0)).toBeFalsy()
  ...
  it('should not insert dupliccates', () => {
    const bstNode = new BinarySearchTreeNode(2)
    bstNode.insert(1)
    expect(bstNode.toString()).toBe('1,2')
    expect(bstNode.contains(1)).toBeTruthy()
  ...
  it('should remove nodes with tow children', () => {
    ...
    bstRootNode.insert(35)
    bstRootNode.remove(25)
    expect(bstRootNode.toString()).toBe('5,10,15,30,35')

    bstRootNode.insert(7)
    expect(bstRootNode.toString()).toBe('5,7,10,15,30,35')

    bstRootNode.remove(5)
    expect(bstRootNode.toString()).toBe('7,10,15,30,35')
  })
  ...
  it('should be possible to use objects as node values', () => {
    ...
    bstRootNode.insert(obj1)

    expect(bstRootNode.toString()).toBe('obj1,obj2')
    expect(bstRootNode.contains(obj1)).toBeTruthy()
    expect(bstRootNode.contains(obj3)).toBeFalsy()

    bstRootNode.insert(obj3)
    expect(bstRootNode.contains(obj3)).toBeTruthy()
    expect(bstRootNode.toString()).toBe('obj1,obj2,obj3')

    expect(bstRootNode.findMin().value).toEqual(obj1)
  })
})

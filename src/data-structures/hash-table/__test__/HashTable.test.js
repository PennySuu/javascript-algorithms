import HasTable from '../HashTable'

describe('HasTable', () => {
  it('should create hash table of certain size', () => {
    const defaultHashTable = new HasTable()
    expect(defaultHashTable.buckets.length).toBe(32)

    const biggerHashTable = new HasTable(64)
    expect(biggerHashTable.buckets.length).toBe(64)
  })
  it('should generate proper has for specified keys', () => {
    const hashTable = new HasTable()
    expect(hashTable.hash('a')).toBe(1)
    expect(hashTable.hash('b')).toBe(2)
    expect(hashTable.hash('abc')).toBe(6)
  })
  it('should set, read and delete data with collisions', () => {
    const hashTable = new HasTable(3)
    expect(hashTable.hash('a')).toBe(1)
    expect(hashTable.hash('b')).toBe(2)
    ...
    expect(hashTable.has('d')).toBeTruthy()
